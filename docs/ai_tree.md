...
| 1 | `runner_sim/zone_sim/user_leaves/` | Auto-discovered package; one file per editor-authored leaf |
| 2 | `ai_tree/server.py` | HTTP endpoints exposing the registry to the editor |
| 2 | `ai_tree/leaf_authoring.py` | Server-side codegen + validation for the New Leaf form |
...
When the system was first introduced, the snapshots were generated by running
the *legacy* `should_extract`/`should_engage` (the `if doctrine == X` branches
that the trees replaced) over the input grid, making the published trees
provably equivalent to the original code at publish time. The legacy code has
since been deleted; the parity test in `tests/test_ai_tree_parity.py` keeps
the snapshots honest by re-evaluating every published tree on its grid and
asserting equality with the saved snapshot file.
...
The primary path is the editor's **New Leaf** form, which writes a generated
file under `runner_sim/zone_sim/user_leaves/<snake>.py` and reloads the
registry without restarting the server. The form takes name, category,
description, required Context fields, parameters, and a Python body
returning bool; the server validates each (PascalCase name, valid identifiers,
body compiles) before writing the file.

The generated file is just normal Python — committed alongside everything
else and hand-editable later:

```python
# runner_sim/zone_sim/user_leaves/low_on_runners.py
...
The autoloader in `user_leaves/__init__.py` discovers each sibling via
`pkgutil.iter_modules` on package import, so a new file is a drop-in: no
central list needs editing.

For foundational leaves you consider part of the core vocabulary, hand-edit
`runner_sim/zone_sim/ai_conditions.py` instead. The runtime makes no
distinction — both files register into the same REGISTRY.
...
than discovering it weeks later in a sim run. See `docs/ai_tree_authoring.md`
for a worked walkthrough of opening a new domain end-to-end.
...
Two more buttons handle the rare-but-needed authoring cases:

- **New Tree** → modal with strict kind + doctrine dropdowns (both sourced
  from runtime vocabulary). Posts to `POST /trees/<name>`, which seeds an
  empty Selector. Used for filling slots you've deleted, or authoring
  trees for a doctrine that was just added to the `Doctrine` enum.
- **New Leaf** → form with name / category / description / requires /
  params / Python body. Posts to `POST /leaves`, which validates the spec,
  writes a generated file under `runner_sim/zone_sim/user_leaves/`, and
  imports it so the registry updates without a restart. The palette
  refreshes automatically; the new leaf is immediately usable in any tree.
...
| `ai_tree/server.py` | ~360 | HTTP endpoints + static handler |
| `ai_tree/leaf_authoring.py` | ~330 | Server-side codegen for New Leaf |
| `runner_sim/zone_sim/ai_conditions.py` | ~140 | The 11 game-specific leaves |
| `runner_sim/zone_sim/user_leaves/` | ~15 | Auto-discovered editor-authored leaves |
| `ai_tree_editor/index.html` + JS + CSS | ~1100 | Browser editor with New Tree / New Leaf |
| `tests/test_ai_tree_*.py + test_leaf_authoring.py` | ~1700 | 252 tests covering all of the above |
